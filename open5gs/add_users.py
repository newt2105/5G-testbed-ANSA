...
def add_user(imsi, key="00112233445566778899aabbccddeeff", op=None,
             opc="63bfa50ee6523365ff14c1f45f88737d", amf="9001", apn="srsapn",
             qci="9", ip_alloc="", sd="000000"):
...
def read_from_db(db_file):
    ...
    for line in db_file:
        if line.startswith("#"):
            ...
            try:
                name, imsi, key, op_type, op_c, amf, qci, ip_alloc, sd = line.split(',')
...
def read_from_string(sub_data):
    ...
    "001010123456780,00112233445566778899aabbccddeeff,opc,
     63bfa50ee6523365ff14c1f45f88737d,8000,9,10.45.1.2,000001"
    '''

    subscriber_db = []

    try:
        imsi, key, op_type, op_c, amf, qci, ip_alloc, sd = sub_data.split(',')
    except ValueError as e:
